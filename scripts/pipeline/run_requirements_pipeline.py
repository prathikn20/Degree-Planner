...
from src.scraper.llm_requirements_parser import parse_rule_text, parse_rule_text_regex

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

DEFAULT_MODEL   = "qwen2.5:14b"
OUTPUT_PATH     = "data/staging/test_degree_requirements.json"
CACHE_PATH      = "data/.cache/req_cache.json"
...
def prefix_concentration_group_ids(groups: list, base_ids: set) -> list:
    """
    Prepend 'conc_' to any concentration group whose ID collides with a base
    group ID.  Called when building concentration blocks so cross-section
    collisions never reach the requirements checker.
    """
    result = []
    for g in groups:
        if g["id"] in base_ids:
...
def validate_track_guardrails(track_id: str, base_core: dict,
                               catalog: dict, logger) -> None:
    """
    Inline guardrails run immediately after assembling each track.
    Logs warnings; never raises (pipeline robustness > strictness at scrape time).
    Use scripts/validate_pipeline_output.py for a strict post-run audit.
    """
    req_set = set(base_core.get("required_courses", []))

    # Guard 1: ghost base required courses
    ghost_req = [c for c in req_set if c not in catalog]
    if ghost_req:
        logger.warning(
            "  [GUARDRAIL] %s: %d ghost base required courses (not in catalog): %s",
            track_id, len(ghost_req), ghost_req,
        )

    # Guard 2: base required credit ceiling — pool-collapse detector
    base_cr = sum(catalog.get(c, {}).get("credits", 0) for c in req_set if c in catalog)
    if base_cr > 150:
        logger.warning(
            "  [GUARDRAIL] %s: base required_courses total %d credits (>150) — likely scraper pool-collapse",
            track_id, base_cr,
        )

    # Guard 3: required vs. options overlap
    for g in base_core.get("choice_groups", []):
        ...
        if stolen:
            ...
            if cr > len(remaining):
                logger.warning(
                    "  [GUARDRAIL] %s/base/%s: %d options also required, leaves only %d valid for %d needed",
                    track_id, g["id"], len(stolen), len(remaining), cr,
                )

    # Guard 4: identical option fingerprints — double-capture detector
    fingerprints: dict[tuple, list] = {}
    for g in base_core.get("choice_groups", []):
        fp = tuple(sorted(g.get("options") or []))
        if fp:
            fingerprints.setdefault(fp, []).append(g["id"])
    for fp, gids in fingerprints.items():
        if len(gids) > 3:
            logger.warning(
                "  [GUARDRAIL] %s: %d groups share identical options %s — double-capture? %s",
                track_id, len(gids), list(fp)[:3], gids,
            )
...
SKIPPED_RULES_PATH    = "data/staging/skipped_rules.json"
MANUAL_PATCHES_PATH   = "data/requirements_manual_patches.json"
...
def run_req_pipeline(model_name: str, force: bool = False, no_llm: bool = False,
    ...
    for track_id, url in TARGET_TRACKS.items():
        ...
        base_group_ids = {g['id'] for g in base_core['choice_groups']}

        # Run inline guardrails — logs warnings without blocking the pipeline
        catalog_for_guard = load_json_file("data/course_catalog.json")
        if catalog_for_guard:
            validate_track_guardrails(track_id, base_core, catalog_for_guard, logger)

        concentrations: dict = {"None": {"required_courses": [], "choice_groups": []}}
        for conc in conc_blocks:
            name = clean_conc_name(conc['block_title'])
            # Auto-prefix concentration group IDs that would shadow base group IDs
            conc['choice_groups'] = prefix_concentration_group_ids(
                reindex_choice_groups(conc['choice_groups']), base_group_ids
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--no-llm', action='store_true',
                        help='Skip LLM rule parsing; log skipped rule texts to data/staging/skipped_rules.json')
