...
- **Updated `_BOILERPLATE_ROW_RE`**: Same dept-descriptor patterns as in req_scraper.py.
...
```
HTML page
  └── req_scraper.scrape_major_requirements()
        └── group_pool_sections()  ← NEW: collapse dept sub-sections
  ↓
  sections list
  └── run_req_pipeline.propagate_reference_lists()  ← NEW: propagate ref_list types
  ↓
  sections with _type injected
  └── req_assembler.assemble_section()
  ...
  ↓
  run_req_pipeline merges blocks → degree_requirements JSON
```
